  ...
  <div class="w3-row orderList">
    ...
    <div class="order-top w3-col s12 l11">
      ...
      <div class="w3-col l3">
        ...
        <input type="text" class="w3-white w3-col l6" @input="getChoiceTypeData('key', $event)">
      </div>
    </div>
...
export default {
  ...
  methods: {
    getTableData () {
      this.axios.get('/api/getOrderList').then((res) => {
        if (res) {
          this.tableData = res.data.data.list
          this.oldTableData = this.tableData.list
        }
      }, (err) => {})
    },
    // 按条件正序或倒序排列
    ...
    getProTypeData (attr) {
      this.tableData = []
      this.oldTableData.forEach((param) => {
        if (param.product === attr.value) {
    ...
    getDateTypeData () {
      ...
      this.tableData = []
      this.oldTableData.forEach((param) => {
        if (Date.parse(param.date) > this.startDate && Date.parse(param.date) < this.endDate) {
          this.tableData.push(param)
        }
      })
    }
